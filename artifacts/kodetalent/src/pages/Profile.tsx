...
import { Award, Share2, X, FileText, Building2, GraduationCap, TrendingUp } from "lucide-react";
...
export default function Profile() {
  ...
  const careerInfoItems = [
    student.cgpa && { icon: GraduationCap, label: "CGPA", value: `${student.cgpa} / 10`, color: "#7c3aed" },
    student.dreamCompany && { icon: Building2, label: "Dream Company", value: student.dreamCompany, color: "#06b6d4" },
    student.targetPackage && { icon: TrendingUp, label: "Target Package", value: student.targetPackage, color: "#10b981" },
  ...
  return (
    <div className="p-4 pb-28 max-w-md mx-auto space-y-5 min-h-screen bg-[#f5f3ff]">
      <div className="flex flex-col items-center justify-center pt-8 pb-4 text-center relative">
        ...
        <p className="text-[#9ca3af] text-sm mt-0.5">{student.field} · Year {student.year}</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <Card className="border-0 bg-white shadow-[0_4px_24px_rgba(124,58,237,0.05)] rounded-2xl">
          <CardContent className="p-4 text-center flex flex-col justify-center items-center h-full">
            ...
            <p className="text-[10px] font-extrabold text-[#6b7280] uppercase tracking-wider">Points</p>
          </CardContent>
        </Card>
        <Card className="border-0 bg-white shadow-[0_4px_24px_rgba(124,58,237,0.05)] rounded-2xl">
          <CardContent className="p-4 text-center flex flex-col justify-center items-center h-full">
      ...
      {careerInfoItems.length > 0 && (
        <Card className="border-0 bg-white shadow-[0_4px_24px_rgba(124,58,237,0.05)] rounded-2xl">
          <CardContent className="p-5">
            <h3 className="font-extrabold text-sm text-[#6b7280] uppercase tracking-wider mb-3">Career Goals</h3>
            <div className="space-y-3">
              {careerInfoItems.map(({ icon: Icon, label, value, color }) => (
                <div key={label} className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-extrabold text-[#9ca3af] uppercase tracking-wider">{label}</p>
        ...
        <Card className="border-0 bg-white shadow-[0_4px_24px_rgba(124,58,237,0.05)] rounded-2xl">
          <CardContent className="p-5">
            ...
            <div className="space-y-4">
              {topSkills.map((skill: { name: string; score: number }, index: number) => {
            ...
            >
              ...
              <div className="p-8 text-center text-white relative z-0">
                ...
                <div className="bg-white/10 rounded-2xl p-5 backdrop-blur-sm border border-white/20 mb-8 text-left">
                  <p className="text-sm font-black text-white mb-4">Score Growth</p>
