    ...
    return (
      <div className="p-4 pb-24 max-w-md mx-auto space-y-6 min-h-screen bg-[#f5f3ff]">
        <div className="text-center space-y-2 mt-10 mb-10">
          <h1 className="text-3xl font-extrabold text-[#1e1b4b]">Test Complete</h1>
          <p className="text-[#6b7280] font-bold">{session.testType}</p>
          <div className="mt-8 flex justify-center">
            <div className="relative w-48 h-48 rounded-full border-[12px] border-[#ede9fe] flex flex-col items-center justify-center bg-white shadow-[0_10px_40px_rgba(124,58,237,0.15)]">
              <span className="text-6xl font-black text-primary">{testResult.score}</span>
              <span className="text-xl font-bold text-[#6b7280]">/{testResult.total}</span>
              <svg className="absolute inset-0 w-full h-full transform -rotate-90 pointer-events-none" style={{ left: '-12px', top: '-12px', width: 'calc(100% + 24px)', height: 'calc(100% + 24px)' }}>
                <circle cx="50%" cy="50%" r="calc(50% - 6px)" fill="transparent" stroke="#7c3aed" strokeWidth="12" strokeDasharray={`${(testResult.score/testResult.total) * 283} 300`} strokeLinecap="round" />
              </svg>
            </div>
          </div>
        </div>
        ...
        <Card className="border-0 shadow-[0_4px_24px_rgba(124,58,237,0.05)] rounded-2xl bg-white">
          <CardContent className="p-5 space-y-5">
            <h3 className="font-extrabold text-[#1e1b4b] text-lg">Breakdown</h3>
            {testResult.sectionBreakdown?.map((section: any, i: number) => (
              <div key={i}>
                <div className="flex justify-between text-sm mb-2">
                  <span className="font-bold text-[#6b7280]">{section.topic}</span>
                  <span className="font-extrabold text-[#1e1b4b]">{section.correct}/{section.total}</span>
                </div>
                <div className="h-2.5 w-full bg-[#ede9fe] rounded-full overflow-hidden">
    ...
    <div className="flex flex-col h-[100dvh] bg-white max-w-md mx-auto relative overflow-hidden">
      <div className="bg-white p-4 sticky top-0 z-10">
        <div className="flex justify-between items-center mb-4">
          <div className="bg-[#ede9fe] text-primary px-3 py-1 rounded-full text-xs font-extrabold">
        ...
        <div className="mb-6">
          <Card className="border-0 shadow-[0_8px_30px_rgba(124,58,237,0.08)] rounded-2xl bg-white mb-6">
            <CardContent className="p-6">
              <span className="inline-block px-3 py-1 bg-[#f5f3ff] text-primary text-[10px] font-extrabold rounded-md mb-4 uppercase tracking-wider">
                {question.topic}
              </span>
              <h2 className="text-xl font-extrabold text-[#1e1b4b] leading-relaxed">{question.question}</h2>
            </CardContent>
          </Card>
                ...
                <motion.div key={idx} whileTap={{ scale: 0.98 }}>
                  <button
                    ...
                    className={`w-full text-left p-5 rounded-2xl border-2 transition-all ${
                      isSelected 
                        ? "border-primary bg-[#f5f3ff] shadow-[0_4px_12px_rgba(124,58,237,0.1)]" 
                        : "border-[#ede9fe] bg-white text-[#6b7280] hover:border-primary/40"
                    }`}
                  >
                    <div className="flex items-center">
                      <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mr-4 shrink-0 transition-colors ${
                        isSelected ? "border-primary bg-primary" : "border-[#6b7280]/40"
                      }`}>
                        {isSelected && <div className="w-2.5 h-2.5 bg-white rounded-full" />}
                      </div>
                      <span className={`text-[15px] font-bold leading-snug ${isSelected ? "text-[#1e1b4b]" : ""}`}>{option}</span>
                    </div>
                  </button>
                </motion.div>
              );
